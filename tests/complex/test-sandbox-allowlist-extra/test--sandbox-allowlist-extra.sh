...
# Complex test: sandbox-allowlist adds extra domains to allowlist file
...
RUN_ID="$(date +%s)-$$"
CONTAINER_NAME="cb-test-sandbox-allowlist-extra-${RUN_ID}"
...
# ---- Test booth -----------------------------------------------------------
TEST_DIR="$(mktemp -d "${SCRIPT_DIR}/.cb-test-sandbox-allowlist-extra.XXXXXX")"
cleanup() {
  rm -rf "$TEST_DIR" || true
}
trap cleanup EXIT
