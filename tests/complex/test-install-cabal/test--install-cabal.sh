...
# -----------------------------------------------------------------------------
# Test: Boothfile `install cabal` sanity check
#
# Verifies that the cabal install manager is reachable end-to-end:
#   1. `install cabal hlint` compiles to RUN cabal--install.sh (known manager)
...
# because some cabal--install.sh scripts are newer than the Docker Hub base.
# -----------------------------------------------------------------------------
...
echo "=== Test: Boothfile install cabal ==="
...
# Test 1: install cabal compiles to RUN cabal--install.sh (cabal is a known manager)
if echo "$DOCKERFILE" | grep -qE "RUN cabal--install\.sh" \
   && ! echo "$DOCKERFILE" | grep -q "Unknown install script 'cabal'"; then
    print_test_result "true" "$0" "1" "install cabal compiles to RUN cabal--install.sh"
else
    print_test_result "false" "$0" "1" "install cabal should compile to RUN cabal--install.sh"
    echo "  Dockerfile: $DOCKERFILE"
    FAILED=$((FAILED + 1))
fi

# The real build needs cabal--install.sh baked into the base image. Build against a
...
if echo "$ACTUAL" | grep -qE 'hlint'; then
    ...
    print_test_result "false" "$0" "2" "hlint is on PATH"
