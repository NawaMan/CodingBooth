...
# -----------------------------------------------------------------------------
# Test: Boothfile `install uv` sanity check
#
# Verifies that the uv install manager is reachable end-to-end:
#   1. `install uv cowsay` compiles to RUN uv--install.sh (known manager)
...
# because some uv--install.sh scripts are newer than the Docker Hub base.
# -----------------------------------------------------------------------------
...
echo "=== Test: Boothfile install uv ==="
...
# Test 1: install uv compiles to RUN uv--install.sh (uv is a known manager)
if echo "$DOCKERFILE" | grep -qE "RUN uv--install\.sh" \
   && ! echo "$DOCKERFILE" | grep -q "Unknown install script 'uv'"; then
    print_test_result "true" "$0" "1" "install uv compiles to RUN uv--install.sh"
else
    print_test_result "false" "$0" "1" "install uv should compile to RUN uv--install.sh"
    echo "  Dockerfile: $DOCKERFILE"
    FAILED=$((FAILED + 1))
fi

# The real build needs uv--install.sh baked into the base image. Build against a
