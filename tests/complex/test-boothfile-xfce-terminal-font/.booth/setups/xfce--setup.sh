...
🚀 What it installs
    ...
    - Font: FiraCode Nerd Font (default terminal font, first-run only)
...
# xfce4-terminal default font — seeded once, like xstartup above. xfce4-terminal
# keeps its font in this per-user file (a plain key file, not xfconf, so there is
# no working system-wide default to seed it from at build time); only writing it
# when the file is still absent means a later font change made from Preferences
# is never overwritten on the next session.
TERM_RC="${HOME}/.config/xfce4/terminal/terminalrc"
if [[ ! -f "$TERM_RC" ]]; then
  mkdir -p "$(dirname "$TERM_RC")"
  cat > "$TERM_RC" <<'RCEOF'
[Configuration]
FontName=FiraCode Nerd Font Mono 11
FontUseSystem=FALSE
RCEOF
fi
