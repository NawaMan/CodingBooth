...
# -----------------------------------------------------------------------------
# Test: Boothfile `install gem` sanity check
#
# Verifies that the gem install manager is reachable end-to-end:
#   1. `install gem colorize` compiles to RUN gem--install.sh (known manager)
...
# because some gem--install.sh scripts are newer than the Docker Hub base.
# -----------------------------------------------------------------------------
...
echo "=== Test: Boothfile install gem ==="
...
# Test 1: install gem compiles to RUN gem--install.sh (gem is a known manager)
if echo "$DOCKERFILE" | grep -qE "RUN gem--install\.sh" \
   && ! echo "$DOCKERFILE" | grep -q "Unknown install script 'gem'"; then
    print_test_result "true" "$0" "1" "install gem compiles to RUN gem--install.sh"
else
    print_test_result "false" "$0" "1" "install gem should compile to RUN gem--install.sh"
    echo "  Dockerfile: $DOCKERFILE"
    FAILED=$((FAILED + 1))
fi

# The real build needs gem--install.sh baked into the base image. Build against a
...
if echo "$ACTUAL" | grep -qE 'colorize'; then
    ...
    print_test_result "false" "$0" "2" "colorize gem is listed"
