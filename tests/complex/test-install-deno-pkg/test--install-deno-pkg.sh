...
# -----------------------------------------------------------------------------
# Test: Boothfile `install deno-pkg` sanity check
#
# Verifies that the deno-pkg install manager is reachable end-to-end:
#   1. `install deno-pkg npm:cowsay` compiles to RUN deno-pkg--install.sh (known manager)
...
# because some deno-pkg--install.sh scripts are newer than the Docker Hub base.
# -----------------------------------------------------------------------------
...
echo "=== Test: Boothfile install deno-pkg ==="
...
# Test 1: install deno-pkg compiles to RUN deno-pkg--install.sh (deno-pkg is a known manager)
if echo "$DOCKERFILE" | grep -qE "RUN deno-pkg--install\.sh" \
   && ! echo "$DOCKERFILE" | grep -q "Unknown install script 'deno-pkg'"; then
    print_test_result "true" "$0" "1" "install deno-pkg compiles to RUN deno-pkg--install.sh"
else
    print_test_result "false" "$0" "1" "install deno-pkg should compile to RUN deno-pkg--install.sh"
    echo "  Dockerfile: $DOCKERFILE"
    FAILED=$((FAILED + 1))
fi

# The real build needs deno-pkg--install.sh baked into the base image. Build against a
...
if echo "$ACTUAL" | grep -qE 'cowsay'; then
    ...
    print_test_result "false" "$0" "2" "cowsay added to deno.json"
