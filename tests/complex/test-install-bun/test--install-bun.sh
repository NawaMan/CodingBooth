...
# -----------------------------------------------------------------------------
# Test: Boothfile `install bun` sanity check
#
# Verifies that the bun install manager is reachable end-to-end:
#   1. `install bun cowsay` compiles to RUN bun--install.sh (known manager)
...
# because some bun--install.sh scripts are newer than the Docker Hub base.
# -----------------------------------------------------------------------------
...
echo "=== Test: Boothfile install bun ==="
...
# Test 1: install bun compiles to RUN bun--install.sh (bun is a known manager)
if echo "$DOCKERFILE" | grep -qE "RUN bun--install\.sh" \
   && ! echo "$DOCKERFILE" | grep -q "Unknown install script 'bun'"; then
    print_test_result "true" "$0" "1" "install bun compiles to RUN bun--install.sh"
else
    print_test_result "false" "$0" "1" "install bun should compile to RUN bun--install.sh"
    echo "  Dockerfile: $DOCKERFILE"
    FAILED=$((FAILED + 1))
fi

# The real build needs bun--install.sh baked into the base image. Build against a
