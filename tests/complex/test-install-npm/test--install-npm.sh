...
# -----------------------------------------------------------------------------
# Test: Boothfile `install npm` sanity check
#
# Verifies that the npm install manager is reachable end-to-end:
#   1. `install npm cowsay` compiles to RUN npm--install.sh (known manager)
...
# because some npm--install.sh scripts are newer than the Docker Hub base.
# -----------------------------------------------------------------------------
...
echo "=== Test: Boothfile install npm ==="
...
# Test 1: install npm compiles to RUN npm--install.sh (npm is a known manager)
if echo "$DOCKERFILE" | grep -qE "RUN npm--install\.sh" \
   && ! echo "$DOCKERFILE" | grep -q "Unknown install script 'npm'"; then
    print_test_result "true" "$0" "1" "install npm compiles to RUN npm--install.sh"
else
    print_test_result "false" "$0" "1" "install npm should compile to RUN npm--install.sh"
    echo "  Dockerfile: $DOCKERFILE"
    FAILED=$((FAILED + 1))
fi

# The real build needs npm--install.sh baked into the base image. Build against a
