...
# -----------------------------------------------------------------------------
# Test: Boothfile `install pip` sanity check
#
# Verifies that the pip install manager is reachable end-to-end:
#   1. `install pip cowsay` compiles to RUN pip--install.sh (known manager)
...
# because some pip--install.sh scripts are newer than the Docker Hub base.
# -----------------------------------------------------------------------------
...
echo "=== Test: Boothfile install pip ==="
...
# Test 1: install pip compiles to RUN pip--install.sh (pip is a known manager)
if echo "$DOCKERFILE" | grep -qE "RUN pip--install\.sh" \
   && ! echo "$DOCKERFILE" | grep -q "Unknown install script 'pip'"; then
    print_test_result "true" "$0" "1" "install pip compiles to RUN pip--install.sh"
else
    print_test_result "false" "$0" "1" "install pip should compile to RUN pip--install.sh"
    echo "  Dockerfile: $DOCKERFILE"
    FAILED=$((FAILED + 1))
fi

# The real build needs pip--install.sh baked into the base image. Build against a
