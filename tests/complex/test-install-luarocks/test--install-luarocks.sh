...
# -----------------------------------------------------------------------------
# Test: Boothfile `install luarocks` sanity check
#
# Verifies that the luarocks install manager is reachable end-to-end:
#   1. `install luarocks busted` compiles to RUN luarocks--install.sh (known manager)
...
# because some luarocks--install.sh scripts are newer than the Docker Hub base.
# -----------------------------------------------------------------------------
...
echo "=== Test: Boothfile install luarocks ==="
...
# Test 1: install luarocks compiles to RUN luarocks--install.sh (luarocks is a known manager)
if echo "$DOCKERFILE" | grep -qE "RUN luarocks--install\.sh" \
   && ! echo "$DOCKERFILE" | grep -q "Unknown install script 'luarocks'"; then
    print_test_result "true" "$0" "1" "install luarocks compiles to RUN luarocks--install.sh"
else
    print_test_result "false" "$0" "1" "install luarocks should compile to RUN luarocks--install.sh"
    echo "  Dockerfile: $DOCKERFILE"
    FAILED=$((FAILED + 1))
fi

# The real build needs luarocks--install.sh baked into the base image. Build against a
...
if echo "$ACTUAL" | grep -qE 'busted'; then
    ...
    print_test_result "false" "$0" "2" "busted rock is listed"
