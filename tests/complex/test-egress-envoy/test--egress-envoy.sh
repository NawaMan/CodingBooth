...
# Complex test: --egress with custom envoy.yaml
...
RUN_ID="$(date +%s)-$$"
CONTAINER_NAME="cb-test-egress-envoy-${RUN_ID}"
...
# ---- Test booth -----------------------------------------------------------
TEST_DIR="$(mktemp -d "${SCRIPT_DIR}/.cb-test-egress-envoy.XXXXXX")"
cleanup() {
  rm -rf "$TEST_DIR" || true
}
trap cleanup EXIT
...
cat > .booth/egress/envoy.yaml <<'EOF'
static_resources:
  listeners:
  - name: egress_proxy
    address:
