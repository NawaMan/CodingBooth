...
# -----------------------------------------------------------------------------
# Test: Boothfile `install conan` sanity check
#
# Verifies that the conan install manager is reachable end-to-end:
#   1. `install conan fmt/10.1.1` compiles to RUN conan--install.sh (known manager)
...
# because some conan--install.sh scripts are newer than the Docker Hub base.
# -----------------------------------------------------------------------------
...
echo "=== Test: Boothfile install conan ==="
...
# Test 1: install conan compiles to RUN conan--install.sh (conan is a known manager)
if echo "$DOCKERFILE" | grep -qE "RUN conan--install\.sh" \
   && ! echo "$DOCKERFILE" | grep -q "Unknown install script 'conan'"; then
    print_test_result "true" "$0" "1" "install conan compiles to RUN conan--install.sh"
else
    print_test_result "false" "$0" "1" "install conan should compile to RUN conan--install.sh"
    echo "  Dockerfile: $DOCKERFILE"
    FAILED=$((FAILED + 1))
fi

# The real build needs conan--install.sh baked into the base image. Build against a
...
if echo "$ACTUAL" | grep -qE 'fmt'; then
    ...
    print_test_result "false" "$0" "2" "fmt package is in the cache"
