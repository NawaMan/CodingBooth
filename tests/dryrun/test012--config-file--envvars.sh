...
ACTUAL=$(run_coding_booth --verbose --dryrun | grep -E '^[A-Z_]+:' | LC_ALL=C sort)
...
ACTUAL=$(run_coding_booth --verbose --dryrun --config test--config.toml | grep -E '^[A-Z_]+:' | LC_ALL=C sort)
