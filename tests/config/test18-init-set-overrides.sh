...
run booth config $prj --no-tui --select "go" --set dind
...
run rm -Rf $prj
mkdir -p $prj
run booth config $prj --no-tui --select "go" --set name=my-booth
...
run rm -Rf $prj
mkdir -p $prj
run booth config $prj --no-tui --select "go" --set daemon=false
...
run rm -Rf $prj
mkdir -p $prj
run booth config $prj --no-tui --select "go" --set keep-alive --set dind --set name=test-booth
...
run rm -Rf $prj
mkdir -p $prj
run booth config $prj --no-tui --select "go" --port 3000 --set port=8080
...
run rm -Rf $prj
mkdir -p $prj
run booth config $prj --no-tui --select "go" --variant base --set keep-alive
...
run rm -Rf $prj
mkdir -p $prj
run booth config $prj --no-tui --select "go" --set dind --set name=test

configfile="$prj/.booth/config.toml"
boothfile="$prj/.booth/Boothfile"
assert-line "$boothfile" "# Configured by: " "booth config --no-tui --overwrite --set dind --set name=test --select go"  "Adjust command includes --set flags"
