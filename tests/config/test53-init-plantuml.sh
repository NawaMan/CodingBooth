...
run rm -Rf $prj
mkdir -p $prj
run booth config $prj --no-tui --select "plantuml:1.2024.8,19090"
boothfile="$prj/.booth/Boothfile"
...
run rm -Rf $prj
mkdir -p $prj
run booth config $prj --no-tui --select "plantuml:1.2025.2,19090+expose"
config="$prj/.booth/config.toml"
...
run rm -Rf $prj
mkdir -p $prj
run booth config $prj --no-tui --select "plantuml:1.2025.2,19090+expose+autostart"
boothfile="$prj/.booth/Boothfile"
config="$prj/.booth/config.toml"
startup="$prj/.booth/startups/65-plantuml-autostart--startup.sh"
