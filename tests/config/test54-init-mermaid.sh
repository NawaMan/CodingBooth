...
run rm -Rf $prj
mkdir -p $prj
run booth config $prj --no-tui --select "mermaid:11.3.0,19190"
boothfile="$prj/.booth/Boothfile"
...
run rm -Rf $prj
mkdir -p $prj
run booth config $prj --no-tui --select "mermaid:11.4.2,19190+expose"
config="$prj/.booth/config.toml"
...
run rm -Rf $prj
mkdir -p $prj
run booth config $prj --no-tui --select "mermaid:11.4.2,19190+expose+autostart"
boothfile="$prj/.booth/Boothfile"
config="$prj/.booth/config.toml"
startup="$prj/.booth/startups/65-mermaid-autostart--startup.sh"
