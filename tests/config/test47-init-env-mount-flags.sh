...
run booth config $prj --no-tui --select "go" --env MY_VAR=hello
...
run rm -Rf $prj
mkdir -p $prj
run booth config $prj --no-tui --select "go" --env FOO=bar --env BAZ=qux
config="$prj/.booth/config.toml"
...
run rm -Rf $prj
mkdir -p $prj
run booth config $prj --no-tui --select "go" --mount /data:/app/data:ro
config="$prj/.booth/config.toml"
...
run rm -Rf $prj
mkdir -p $prj
run booth config $prj --no-tui --select "go" --mount /data:/app/data --mount /logs:/var/log
config="$prj/.booth/config.toml"
...
run rm -Rf $prj
mkdir -p $prj
run booth config $prj --no-tui --select "go" --expose 8080 --env FOO=bar --mount /tmp:/tmp
config="$prj/.booth/config.toml"
...
run rm -Rf $prj
mkdir -p $prj
run booth config $prj --no-tui --env DB_HOST=localhost
config="$prj/.booth/config.toml"
...
run rm -Rf $prj
mkdir -p $prj
run booth config $prj --no-tui --mount /data:/data
config="$prj/.booth/config.toml"
...
run rm -Rf $prj
mkdir -p $prj
run booth config $prj --no-tui --select "go" --env FOO=bar --mount /data:/data
boothfile="$prj/.booth/Boothfile"
assert-line "$boothfile" "# Configured by: " "booth config --no-tui --overwrite --env FOO=bar --mount /data:/data --select go"  "Adjust command includes --env and --mount"
