...
| `new [path]`     | Generate a new project (default: current directory)           |
| `adjust [path]`  | Re-generate project (overwrites existing files)               |
| `dryrun`         | Print what would be generated without writing files           |

> **Note:** `dryrun` is a subcommand (not a `--dryrun` flag) because it does not require a target location. The `new` and `adjust` commands default to the current directory if no path is provided.

**Target location safety:** The `new` subcommand allows the `.booth/` directory to already exist, but if any files to be generated already exist, it stops and asks the user to confirm overwrite. The `--overwrite` flag (or the `adjust` subcommand) skips this confirmation. This prevents accidental loss of manual customizations while still supporting re-initialization.
...
| `--overwrite`             | Overwrite existing files without prompting (for new). Used implicitly by `adjust`.   |
...
# Generated by: booth init new --select go/java:25,temurin
# Adjust with : booth init adjust --select go/java:25,temurin
