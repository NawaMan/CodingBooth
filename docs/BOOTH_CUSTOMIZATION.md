...
You can also install packages without editing the Boothfile directly — use the **package manager templates** with `booth config`:

```bash
# Install global npm packages
./booth config --no-tui ../project --select "nodejs+npm-pkg:express,typescript"

# Install global pip packages
./booth config --no-tui ../project --select "python+pip-pkg:flask,httpx"

# Pre-install project dependencies from manifest files
./booth config --no-tui ../project --select "nodejs+npm-install"    # reads package.json at build time
./booth config --no-tui ../project --select "java+maven+mvn-install" # reads pom.xml at build time
```
...
A **template** is a reusable, composable unit of CodingBooth configuration. When you run `./booth config --no-tui ../project --select go/python`, the `go` and `python` templates are merged to produce a complete `.booth/` folder — Boothfile, config.toml, startup scripts, and any supporting files.

**Why templates?**

Writing `.booth/` configuration by hand is manageable for simple projects. But for a polyglot project with multiple languages, AI tools, credential seeding, IDE extensions, and databases, you'd need to know the correct setup script arguments, segment ordering, volume mount syntax, and various other details. Templates encode all of this, and the config compiler handles the merge logic.
...
Startup segments follow the same pattern: `"startup.sh"` (order 50), `"startup--90.sh"` (order 90), etc. Each startup segment becomes an individual file in `.booth/startups/` named `NN-name--startup.sh` (e.g., `65-excalidraw-autostart--startup.sh`). Users can add their own `*--startup.sh` files to `startups/` — files without a `NN-` prefix default to order 50, and files without a `# Generated by:` header survive `booth config --no-tui --overwrite`.
...
# Inline heredoc
./booth config --no-tui ../my-project --select - <<RECIPE
go
python:3.13
  + uv
claude-code
