...
**Important:** `booth init` always initializes a **different** project folder, not the current one. The current folder must already have a `.booth/` (that's how `booth` itself runs). The target folder must either not exist or not contain `.booth/` — init never overwrites existing configuration.
...
Safety enforced:
- `.booth/` must not already exist at the target path.
...
- **Target safety** — `new` refuses to overwrite an existing `.booth/` directory.
