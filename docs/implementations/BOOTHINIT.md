...
> `./booth config --no-tui ../my-project --select go+linter/python:3.13+uv/claude-code` creates a complete `.booth/` configuration — Boothfile, config, startup scripts — ready to launch.
...
`booth config` runs **on the host** via the `codingbooth` binary — no Docker required. This is intentional:
...
```
./booth config --no-tui ../my-project --select go
    │
    └─► codingbooth config --no-tui ../my-project --select go
            │
...
**Important:** `booth config` always initializes a **different** project folder, not the current one. The current folder must already have a `.booth/` (that's how `booth` itself runs). The target folder's `.booth/` directory may already exist — config will prompt for confirmation if any generated files would overwrite existing ones (use `--overwrite` to skip the prompt).
...
Safety enforced:
- If generated files already exist, config prompts for confirmation (unless `--overwrite`).
- Files are only written when their content is non-empty.
- Startup files in `startups/` get `chmod 755`; other files get `0644`.
- On `--overwrite`, generated startup files (with `# Generated by:` header) are cleaned up before writing new ones. User-added files are preserved.
...
```
# Generated by: booth config --no-tui --version 0.21.0 --select go/python
# Adjust with : booth config --no-tui --overwrite --version 0.21.0 --select go/python
```
...
Alternatively, segments can be defined as separate files: `Boothfile`, `Boothfile--60`, `startup.sh`, `startup--90.sh`. The numeric suffix controls ordering (default is 50). Startup segments become individual files in `.booth/startups/` (e.g., `50-go--startup.sh`, `90-go--startup.sh`).
...
Segments are the building blocks of the generated Boothfile and startup scripts. When multiple templates are selected, all their segments are sorted globally:
...
### 1. Concatenation (Boothfile) / Individual files (startups/)

Boothfile segments from all templates are concatenated in order. Startup segments become individual files in `.booth/startups/`, named `NN-name--startup.sh`.
...
./booth config --no-tui ../my-project --select go+linter/python:3.13+uv/claude-code
./booth config --no-tui --select python --port 10080
./booth config --no-tui --select python --expose 8080 --env MY_VAR=hello --mount /data:/app/data
./booth config --no-tui --select go --set keep-alive --set name=my-booth
./booth config --no-tui                                  # Empty booth (no templates, just CLI overrides)
```

The path defaults to the current directory if omitted. Running `config --no-tui` without `--select` creates an empty `.booth/` with only CLI overrides applied (e.g., `--variant`, `--port`, `--set`).

### Re-generate (Overwrite)

```bash
./booth config --no-tui --overwrite --select go+linter/python:3.13+uv/claude-code+django
```

The `--overwrite` flag overwrites existing files without prompting.
...
| `--start` | Launch `codingbooth run --code <path>` after config |
...
- **Target safety** — config prompts for confirmation before overwriting existing files (use `--overwrite` to skip).
...
cli/src/
├── cmd/codingbooth/
│   ├── init.go                  # CLI entry point for config (--no-tui, --overwrite, --dryrun)
...
└── pkg/boothinit/
    ...
    │   ├── startup.go           # Startup file serialization (shebang + generated header)
