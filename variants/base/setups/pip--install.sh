...
# No pip packages requested is a no-op, not an error: the *-pkg templates emit
# `install pip ${..._PKGS}` with the package list defaulting to empty, so
# failing here would break the image build of every project that selects the
# extension without naming packages.
if [ $# -eq 0 ]; then
    echo "ℹ️  No pip packages requested; nothing to install."
    exit 0
fi
...
cb_retry /opt/python/bin/pip install "$@"
