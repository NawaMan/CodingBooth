...
if [[ "$REQ_VER" == "latest" ]]; then
  VERSION=$(curl -fsSL https://api.github.com/repos/nektos/act/releases/latest | grep -oE '"tag_name"[[:space:]]*:[[:space:]]*"v?[^"]+"' | head -1 | sed -E 's/.*"v?([^"]+)".*/\1/')
else
  VERSION="${REQ_VER#v}"
fi
...
curl -fsSL "https://github.com/nektos/act/releases/download/v${VERSION}/act_Linux_${ACT_ARCH}.tar.gz" -o "$TMP/act.tar.gz"
