...
# gcc-code-extension--setup.sh
# NOTE: This script has not been tested -- no time (sorry). Please report success or failure. :-p
# Installs Microsoft's C/C++ extension (cpptools), the canonical VS Code C/C++ pack
# that pairs well with GCC. For a clangd-based experience use clang-code-extension instead.
set -Eeuo pipefail
