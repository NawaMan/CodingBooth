...
func TestIntegration_InitializeAppContext_Sandbox_ValidAllowlist(t *testing.T) {
...
`,
	...
	if got := res.Ctx.SandboxMode(); got != "envoy" {
		t.Fatalf("expected normalized sandbox mode %q, got %q", "envoy", got)
	}
	if got := res.Ctx.SandboxEnforcement(); got != "iptables" {
...
func TestIntegration_InitializeAppContext_Sandbox_FlagSetsDefaults(t *testing.T) {
	res := RunInitializeAppContext(t, TestInput{
	...
	if got := res.Ctx.SandboxMode(); got != "envoy" {
		t.Fatalf("expected sandbox mode %q, got %q", "envoy", got)
	}
	if got := res.Ctx.SandboxEnforcement(); got != "iptables" {
		t.Fatalf("expected sandbox enforcement %q, got %q", "iptables", got)
	}
