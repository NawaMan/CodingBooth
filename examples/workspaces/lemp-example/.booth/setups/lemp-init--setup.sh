...
# Workspace-local setup for the LEMP example:
# - Installs php-mysql so PHP can talk to MySQL.
# - Drops the example index.php into /var/www/html.
# - Adds a startup hook that creates the `lemp_demo` database with one row.
...
$mysqli = new mysqli('localhost', $user, '', 'lemp_demo');
if ($mysqli->connect_errno) {
    http_response_code(500);
    echo "<h1>LEMP example — DB not ready yet</h1>";
...
<html><body style="font-family:system-ui;padding:2rem">
  <h1>Hello from LEMP in CodingBooth!</h1>
  <p>nginx served this page, PHP-FPM rendered it, MySQL gave us this row:</p>
...
STARTUP_FILE="/usr/share/startup.d/60-cb-lemp--startup.sh"
...
CREATE DATABASE IF NOT EXISTS lemp_demo;
USE lemp_demo;
CREATE TABLE IF NOT EXISTS hello (id INT AUTO_INCREMENT PRIMARY KEY, message VARCHAR(255));
INSERT INTO hello (message) SELECT 'Hi from MySQL via PHP-FPM!' WHERE NOT EXISTS (SELECT 1 FROM hello);
SQL
STARTUP
chmod 755 "$STARTUP_FILE"

echo "✅ LEMP demo ready: index.php + lemp_demo database with one row."
