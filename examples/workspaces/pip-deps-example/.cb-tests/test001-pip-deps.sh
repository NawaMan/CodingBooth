...
output=$("$BOOTH" --variant base --port "${CB_PORT:-50232}" -- 'python3 -c "import requests; print('"'"'OK'"'"')"' 2>&1) || true
...
output=$("$BOOTH" --variant base --port "${CB_PORT:-50232}" -- 'python3 -c "import flask; print('"'"'OK'"'"')"' 2>&1) || true
if grep -q "OK" <<< "$output"; then
