#!/bin/bash
set -e
# Configured by: booth config --no-tui --overwrite --variant xfce --select go+vscode-ext+kernel/python+pip-config+vscode-ext/notebook/codeserver/claude-code+auto-accept+credential+settings-cache
